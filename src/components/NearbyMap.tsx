      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (pos) => {
          ...
          try {
            ...
            const nearby = users
              .filter((u: any) => u.coordinates)
              .map((u: any) => {
                const userCoords = JSON.parse(u.coordinates);
                const distance = calculateDistance(latitude, longitude, userCoords.lat, userCoords.lng);
                return {
                  id: u.id,
                  name: u.display_name,
                  age: u.age || 25,
                  ...
                  lat: userCoords.lat,
                  lng: userCoords.lng,
                  img: u.photo_url || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100'
                };
              })
              .filter((u: any) => parseFloat(u.dist) < 50) // Within 50 miles
              .sort((a: any, b: any) => parseFloat(a.dist) - parseFloat(b.dist));

            setUserNearby(nearby);
          } catch (err) {
            console.error(err);
          } finally {
            setLoading(false);
          ...
          setLoading(false);
          setLocationName("Austin, TX (Demo)");
          // Demo fallback
          setUserNearby([
            { id: 'demo1', name: 'Elizabeth', dist: '0.4 mi', age: 24, lat: 30.2672 + 0.003, lng: -97.7431 - 0.002, img: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100' },
            { id: 'demo2', name: 'Sarah', dist: '0.7 mi', age: 26, lat: 30.2672 - 0.004, lng: -97.7431 + 0.003, img: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?w=100' }
          ]);
        });
      }
    };
             ...
             <div className="absolute inset-0 z-0">
                <div className="w-full h-full bg-[#0a0a14] relative overflow-hidden">
                   ...
                   {userNearby.map((user, idx) => {
                      // Calculate relative position for simulation if real map isn't integrated
                      // In a real app, we'd use a Map library like Mapbox or Google Maps
                      // For this UI, we offset markers from the center based on their lat/lng diff
                      const latDiff = (user.lat - coords.lat) * 2000;
                      const lngDiff = (user.lng - coords.lng) * 2000;
