...
const MatchmakerDemo: React.FC<{ onNavigate?: (page: any) => void }> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (user) {
        const profile = await (blink.db as any).users.get(user.id);
  ...
  const runAlgorithm = async () => {
    ...
    try {
      ...
      const users = await (blink.db as any).users.list();
      ...
      const freshUsers = await (blink.db as any).users.list();
      const matches = freshUsers
        .filter((u: any) => u.user_id !== authUser?.id && u.gender === prefs.gender)
        .map((u: any) => calculateMatchScore(authUser, u, prefs));
      
      setResults(matches.sort((a, b) => b.score - a.score));
      
      // Fetch user's likes
      const myLikes = await (blink.db as any).likes.list({ user_id: authUser.id });
      setLikedProfiles(new Set(myLikes.map((l: any) => l.target_user_id)));
    } catch (error) {
      console.error(error);
  ...
  const seedUsers = async () => {
    const seedData = [
      { id: 'seed1', user_id: 'seed1', display_name: 'Elizabeth', email: 'elizabeth@example.com', gender: 'Woman', location: 'Austin, TX', faith_level: 9, values_level: 9, intention_level: 9, lifestyle_level: 9, photo_url: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=800&q=80', bio: 'Saving myself for marriage.' },
      { id: 'seed2', user_id: 'seed2', display_name: 'Sarah', email: 'sarah@example.com', gender: 'Woman', location: 'Dallas, TX', faith_level: 7, values_level: 8, intention_level: 7, lifestyle_level: 6, photo_url: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?auto=format&fit=crop&w=800&q=80', bio: 'Love Jesus and coffee.' },
      { id: 'seed3', user_id: 'seed3', display_name: 'Mary', email: 'mary@example.com', gender: 'Woman', location: 'Houston, TX', faith_level: 10, values_level: 10, intention_level: 10, lifestyle_level: 10, photo_url: 'https://images.unsplash.com/photo-1524504388940-b1c1722653e1?auto=format&fit=crop&w=800&q=80', bio: 'Traditional Catholic.' },
    ];
    for (const u of seedData) {
      await (blink.db as any).users.create(u);
    }
  };

  const handleLike = async (e: React.MouseEvent, targetId: string) => {
    e.stopPropagation();
    const authUser = await blink.auth.me();
    if (!authUser) {
    ...
    try {
      if (likedProfiles.has(targetId)) {
        // Find the like record and delete it
        const myLikes = await (blink.db as any).likes.list({ 
          user_id: authUser.id,
          target_user_id: targetId 
        });
        if (myLikes.length > 0) {
          await (blink.db as any).likes.delete(myLikes[0].id);
        ...
        await (blink.db as any).likes.create({
          user_id: authUser.id,
          target_user_id: targetId
        });
        ...
        const theirLikes = await (blink.db as any).likes.list({
          user_id: targetId,
        ...
        if (theirLikes.length > 0) {
          // It's a match!
          await (blink.db as any).matches.create({
            user_id: authUser.id,
            partner_user_id: targetId
          });
          toast.success("It's a Covenant Match! Start a conversation.", {
            icon: '❤️',
            duration: 5000
          });
        } else {
